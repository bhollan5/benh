...
<div id="container" v-if="answers">
  <cognoteHeader></cognoteHeader>
    ...
    <div id="results" style="height: 80%; margin-top: 5%;">
        <p>Results: {{ $parent.correct }} / {{ $parent.questions.length }}</p>
        <p v-for="index in $parent.questions.length">
            Question {{index}} : {{ $parent.questions[index - 1].status }}
...
export default {
  ...
  calculated: {
      percentage: function () {
          var perc = Math.floor((this.$parent.correct / this.$parent.questions.length) * 100);
          console.log(perc);
          return perc;
      }
  },
  ...
  mounted: function() {
      if (this.$parent.questions[0].status == 'none') {
          this.answers = false;
      }
      var vm = this;
      setTimeout(function(){ 
      ...
      this.perc = Math.floor((this.$parent.correct / this.$parent.questions.length) * 100);
  ...
  methods: {
      ...
      redoQuiz: function(incorrectOnlyBool) {
          this.$parent.incorrectOnly = incorrectOnlyBool;
          this.$parent.changeCogRoute('quiz');
      }
  }
}
